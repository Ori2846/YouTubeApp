...
##     def build_comserver(self, target, templates, arcname, boot_script="com_servers"):
...
##         # and build it
##         return self.build_executable(target, templates, arcname, boot, vars)
...
##     def build_service(self, target, templates, arcname):
...
##     def build_isapi(self, target, templates, arcname):
##         target_module = os.path.splitext(os.path.basename(target.script))[0]
##         vars = {"isapi_module_name" : target_module,
##                }
##         return self.build_executable(target, templates, arcname, None, vars)

##     def build_manifest(self, target, templates):
...
##         if os.path.splitext(templates)[1]==".exe":
##             rid = 1
##         else:
##             rid = 2
##         try:
##             # Manfiests have resource type of 24, and ID of either 1 or 2.
##             mfest = load_resource(ensure_unicode(templates), RT_MANIFEST, rid)
...
##             # in this case the templates had no existing manifest, so its
...
##     def build_executable(self, target, templates, arcname, script, vars={}):
##         # Build an executable for the target
##         # templates is the exe-stub to use, and arcname is the zipfile
##         # containing the python modules.
##         from py2exe_util import add_resource, add_icon
##         ext = os.path.splitext(templates)[1]
...
##         # will see the earlier time of our 'templates' file versus the later
##         # time of our modified templates file, and consider our old file OK.
