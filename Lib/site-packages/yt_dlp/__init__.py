...
def validate_options(opts):
    ...
    for k, tmpl in opts.outtmpl.items():
        validate_outtmpl(tmpl, f'{k} output templates')
    for type_, tmpl_list in opts.forceprint.items():
        for tmpl in tmpl_list:
            validate_outtmpl(tmpl, f'{type_} print templates')
    for type_, tmpl_list in opts.print_to_file.items():
        for tmpl, file in tmpl_list:
            validate_outtmpl(tmpl, f'{type_} print to file templates')
    ...
    for k, tmpl in opts.progress_template.items():
        ...
        validate_outtmpl(tmpl, f'{k} templates')
    ...
    if outtmpl_default and not os.path.splitext(outtmpl_default)[1] and opts.extractaudio:
        raise ValueError(
            ...
            f'Use "{outtmpl_default}.%(ext)s" instead of "{outtmpl_default}" as the output templates')
